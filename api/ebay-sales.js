  ...
  try {
    ...
    while (hasMore) {
      ...
      if (!ordersResponse.ok) {
        ...
        return res.status(ordersResponse.status).json({ error: 'Failed to fetch orders', details: errorText, url: ordersUrl });
      }
          ...
          if (salesData.transactions) {
            for (const tx of salesData.transactions) {
              if (tx.orderId && tx.amount) {
                salesTxMap.set(tx.orderId, { payout: parseFloat(tx.amount.value || 0), totalFees: parseFloat(tx.totalFeeAmount?.value || 0), transactionId: tx.transactionId });
              }
            }
            if (salesData.transactions.length < limit || txOffset >= 1000) { txHasMore = false; } else { txOffset += limit; }
          } else { txHasMore = false; }
        } else { txHasMore = false; }
      }
    } catch (e) { console.log('[eBay API] Finances SALE API failed:', e.message); }
      ...
      while (chargeHasMore) {
        ...
        if (chargesResponse.ok) {
          ...
          if (chargesData.transactions) {
            ...
            if (chargesData.transactions.length < limit || chargeOffset >= 1000) { chargeHasMore = false; } else { chargeOffset += limit; }
          } else { chargeHasMore = false; }
        } else { chargeHasMore = false; }
      }
    } catch (e) { console.log('[eBay API] Finances NON_SALE_CHARGE API failed:', e.message); }

    const uniqueItemIds = [...new Set(allOrders.flatMap(o => (o.lineItems || []).map(li => li.legacyItemId).filter(Boolean)))];
    ...
    for (const order of allOrders) {
      for (const li of order.lineItems || []) {
        ...
        const img = li.image?.imageUrl || li.imageUrl || li.pictureURL || li.galleryURL || li.thumbnailImageUrl || li.mainImage?.imageUrl || (li.images && li.images[0]?.imageUrl);
      ...
      await Promise.all(batch.map(async (itemId) => {
        ...
        try {
          const r = await fetch('https://api.ebay.com/ws/api.dll', {
            method: 'POST',
            headers: { 'X-EBAY-API-SITEID': '0', 'X-EBAY-API-COMPATIBILITY-LEVEL': '1225', 'X-EBAY-API-CALL-NAME': 'GetItem', 'X-EBAY-API-IAF-TOKEN': accessToken, 'Content-Type': 'text/xml' },
    ...
    if (remaining1.length > 0 && clientId) {
      for (let i = 0; i < remaining1.length; i += 20) {
        ...
        try {
          const shopRes = await fetch(`https://open.api.ebay.com/shopping?callname=GetMultipleItems&responseencoding=JSON&appid=${clientId}&siteid=0&version=967&ItemID=${batch.join(',')}&IncludeSelector=Details,Variations`);
          if (shopRes.ok) {
            const data = await shopRes.json();
            const items = Array.isArray(data.Item) ? data.Item : (data.Item ? [data.Item] : []);
            for (const item of items) {
              if (item.ItemID && !imageMap.has(item.ItemID)) {
                const pic = (item.PictureURL && item.PictureURL[0]) || item.GalleryURL;
    ...
    for (let i = 0; i < remaining2.length; i += 10) {
      ...
      await Promise.all(batch.map(async (itemId) => {
        if (imageMap.has(itemId)) return;
        try {
          const r = await fetch(`https://api.ebay.com/buy/browse/v1/item/get_item_by_legacy_id?legacy_item_id=${itemId}`, { headers: { 'Authorization': `Bearer ${accessToken}`, 'X-EBAY-C-MARKETPLACE-ID': 'EBAY_US' } });
          if (r.ok) {
    ...
    for (const order of allOrders) {
      ...
      for (const lineItem of order.lineItems || []) {
        ...
        if (salesTxMap.has(order.orderId)) {
          ...
          payout = adFee > 0 ? payout - adFee : payout;
          payoutSource = adFee > 0 ? 'Finances API (minus AD_FEE)' : 'Finances API';
        } else {
          payout = parseFloat(order.paymentSummary?.totalDueSeller?.value || 0);
          payout = adFee > 0 ? payout - adFee : payout;
          payoutSource = adFee > 0 ? 'Order API (minus AD_FEE)' : 'Order API';
        }
        ...
        sales.push({
          ...
          item_id: itemId,
          _raw: { orderId: order.orderId, orderPaymentStatus: order.orderPaymentStatus, creationDate: order.creationDate }
        });
      }
    }
    ...
    res.status(200).json({
      success: true, sales, count: sales.length, imagesFound: imageMap.size, totalItems: uniqueItemIds.length,
      summary: {
        totalRevenue: totalRevenue.toFixed(2), totalFees: totalFees.toFixed(2),
        totalAdFees: totalAdFees.toFixed(2), totalPayout: totalPayout.toFixed(2),
    ...
    res.status(500).json({ error: 'Failed to fetch eBay sales', message: err.message });
  }
}
