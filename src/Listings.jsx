...
export default function Listings({ stockxToken, ebayToken, purchases = [], c = { bg: '#0a0a0a', card: '#111111', border: '#1a1a1a', text: '#ffffff', textMuted: '#888888', gold: '#C9A962', green: '#10b981', red: '#ef4444' } }) {
  ...
  const [marketData, setMarketData] = useState(() => { try { return JSON.parse(localStorage.getItem('fl_md') || '{}'); } catch { return {}; } });
  ...
  const fetchMarketData = useCallback(async (product) => {
    if (!stockxToken || !product?.sizes?.length) return;
    const productId = product.sizes[0]?.productId;
    const variantIds = [...new Set(product.sizes.map(s => s.variantId))].join(',');
    if (!productId || !variantIds) return;
    const cacheKey = `md_${productId}`;
    if (marketData[cacheKey]?.ts && Date.now() - marketData[cacheKey].ts < 300000) return;
    try {
      const res = await fetch(`/api/stockx-listings?productId=${productId}&variantIds=${variantIds}`, { headers: { 'Authorization': `Bearer ${stockxToken}` } });
      if (res.ok) { const d = await res.json(); const newMD = { ...marketData, [cacheKey]: { ...d.marketData, ts: Date.now() } }; setMarketData(newMD); localStorage.setItem('fl_md', JSON.stringify(newMD)); }
    } catch {}
  }, [stockxToken, marketData]);

  useEffect(() => { if ((stockxToken || ebayToken) && !stockxListings.length) syncListings(); }, []);

  const groupedProducts = useMemo(() => {
    const g = {};
    stockxListings.forEach(l => { const sku = l.sku || 'UNK'; if (!g[sku]) g[sku] = { sku, name: l.name, image: l.image, productId: l.productId, sizes: [] }; g[sku].sizes.push({ ...l, cost: getCost(sku, l.size) }); });
    Object.values(g).forEach(p => { p.sizes.sort((a, b) => parseFloat(a.size) - parseFloat(b.size)); p.totalQty = p.sizes.length; p.notLowest = 0; });
  ...
  const currentMD = useMemo(() => { if (!currentProduct?.sizes?.length) return {}; return marketData[`md_${currentProduct.sizes[0]?.productId}`] || {}; }, [currentProduct, marketData]);

  useEffect(() => { if (filteredProducts.length && !selectedProduct) setSelectedProduct(filteredProducts[0].sku); }, [filteredProducts]);
  useEffect(() => { if (currentProduct) fetchMarketData(currentProduct); }, [currentProduct]);
  ...
  const handleUpdatePrices = async () => { const u = Object.entries(editedPrices).map(([id, a]) => ({ listingId: id, amount: Math.round(parseFloat(a)) })).filter(x => x.amount > 0); if (!u.length) return; setLoading(true); try { const r = await fetch('/api/stockx-listings', { method: 'PATCH', headers: { 'Authorization': `Bearer ${stockxToken}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ items: u }) }); if (r.ok) { showToast(`Updated ${u.length}`); setEditedPrices({}); await syncListings(); } else showToast('Failed', 'error'); } catch { showToast('Failed', 'error'); } finally { setLoading(false); } };
  const handleUnlist = async () => { if (!selectedSizes.size) return; setLoading(true); try { const r = await fetch('/api/stockx-listings', { method: 'DELETE', headers: { 'Authorization': `Bearer ${stockxToken}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ listingIds: Array.from(selectedSizes) }) }); if (r.ok) { showToast(`Unlisted ${selectedSizes.size}`); setSelectedSizes(new Set()); await syncListings(); } else showToast('Failed', 'error'); } catch { showToast('Failed', 'error'); } finally { setLoading(false); } };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        {[{ id: 'reprice', icon: '⚡', label: 'Reprice', count: stockxListings.length }, { id: 'crosslist', icon: '🚀', label: 'Cross-list', count: totalCrosslist }, { id: 'all', icon: '📦', label: 'All Listings', count: stockxListings.length + ebayListings.length }].map(t => (
            ...
            <div style={{ maxHeight: 540, overflowY: 'auto' }}>
              {filteredProducts.map(p => (
                <div key={p.sku} onClick={() => { setSelectedProduct(p.sku); setSelectedSizes(new Set()); setEditedPrices({}); }} style={{ padding: '16px 20px', borderBottom: `1px solid ${c.border}`, cursor: 'pointer', background: selectedProduct === p.sku ? 'rgba(255,255,255,0.05)' : 'transparent', borderLeft: selectedProduct === p.sku ? `4px solid ${c.gold}` : '4px solid transparent', display: 'flex', gap: 14, alignItems: 'center' }}>
                  <div style={{ width: 48, height: 48, background: 'rgba(255,255,255,0.05)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 }}>{p.image ? <img src={p.image} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: 24 }}>👟</span>}</div>
            ...
            {currentProduct ? (
              <>
                <div style={{ padding: '20px 24px', borderBottom: `1px solid ${c.border}`, display: 'flex', gap: 16, alignItems: 'center' }}>
                  <div style={{ width: 60, height: 60, background: 'rgba(255,255,255,0.05)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>{currentProduct.image ? <img src={currentProduct.image} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: 30 }}>👟</span>}</div>
                  <div>
                    <h3 style={{ margin: 0, fontSize: 20, fontWeight: 700 }}>{currentProduct.name}</h3>
                    <div style={{ fontSize: 13, color: c.textMuted, marginTop: 4 }}>{currentProduct.sku} {currentProduct.sizes[0]?.inventoryType === 'DIRECT' && <span style={{ marginLeft: 8, background: '#f97316', color: '#fff', padding: '3px 10px', borderRadius: 6, fontSize: 11, fontWeight: 700 }}>🚀 Direct</span>}</div>
                  </div>
                </div>
                <div style={{ padding: '12px 24px', borderBottom: `1px solid ${c.border}`, background: 'rgba(255,255,255,0.02)' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', fontSize: 13, color: c.textMuted }}><input type="checkbox" checked={currentProduct.sizes.length > 0 && selectedSizes.size === currentProduct.sizes.length} onChange={handleSelectAll} style={{ width: 16, height: 16, accentColor: c.green }} /> Select all</label>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '44px 80px 70px 110px 110px 110px 1fr', padding: '14px 24px', borderBottom: `1px solid ${c.border}`, background: 'rgba(255,255,255,0.02)', fontSize: 11, fontWeight: 700, color: c.textMuted }}>
                  <span></span><span>SIZE</span><span style={{ textAlign: 'center' }}>QTY</span><span style={{ textAlign: 'center' }}>YOUR ASK</span><span style={{ textAlign: 'center' }}>LOWEST ASK</span><span style={{ textAlign: 'center' }}>SELL FASTER</span><span style={{ textAlign: 'right' }}>COST</span>
                </div>
                <div style={{ flex: 1, overflowY: 'auto', maxHeight: 360 }}>
                  {currentProduct.sizes.map(item => {
                    const md = currentMD[item.variantId] || {};
                    const lowest = md.lowestAsk || item.lowestAsk;
                    const sellFast = md.sellFaster || item.sellFaster || item.highestBid;
                    const isLowest = lowest && item.yourAsk <= lowest;
                    ...
                    return (
                      <div key={item.listingId} style={{ display: 'grid', gridTemplateColumns: '44px 80px 70px 110px 110px 110px 1fr', padding: '16px 24px', borderBottom: `1px solid ${c.border}`, alignItems: 'center', fontSize: 14 }}>
                        ...
                        <div style={{ textAlign: 'center' }}><input type="number" value={editedPrices[item.listingId] ?? item.yourAsk} onChange={e => setEditedPrices({ ...editedPrices, [item.listingId]: e.target.value })} style={{ width: 80, padding: '10px', background: isEdited ? 'rgba(201,169,98,0.2)' : 'rgba(255,255,255,0.05)', border: `1px solid ${isEdited ? c.gold : c.border}`, borderRadius: 8, color: c.text, fontSize: 14, textAlign: 'center' }} /></div>
                        <div style={{ textAlign: 'center', color: isLowest ? c.green : c.text, fontWeight: 600 }}>{lowest ? `$${lowest}` : '—'}{isLowest && <span style={{ marginLeft: 6 }}>✓</span>}</div>
                        <span style={{ textAlign: 'center', color: '#f97316', fontWeight: 600 }}>{sellFast ? `$${sellFast}` : '—'}</span>
      ...
      {subTab === 'crosslist' && <div style={{ ...card, padding: 80, textAlign: 'center' }}><div style={{ fontSize: 56 }}>🚀</div><h3 style={{ marginTop: 16 }}>Cross-list to eBay</h3><p style={{ color: c.textMuted }}>{crosslistProducts.length} products not on eBay</p></div>}
