...
export default function Analytics({ sales, purchases, year, c, fmt }) {
  ...
  const skuData = useMemo(() => {
    ...
    filteredSales.forEach(s => {
      ...
      if (!map[key]) {
        map[key] = { sku: s.sku || '', name: s.name || 'Unknown', units: 0, totalCost: 0, totalSale: 0, totalProfit: 0 };
      }
      map[key].units++;
  ...
  const cols = '2.5fr 0.6fr 0.9fr 0.9fr 0.9fr 1.1fr';
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20, marginBottom: 28 }}>
        {[
          ...
          { label: 'Best SKU', value: bestSku?.sku || '—', color: '#8B5CF6' },
        ].map((stat, i) => (
          <div key={i} style={{ background: c.card, border: `1px solid ${c.border}`, borderRadius: 20, padding: 28, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
        <input
          ...
          style={{ flex: 1, minWidth: 200, padding: 14, background: 'rgba(255,255,255,0.03)', border: `1px solid ${c.border}`, borderRadius: 12, color: '#fff', fontSize: 14, outline: 'none' }}
        />
        <select
          value={platform}
      ...
      <div style={{ background: c.card, border: `1px solid ${c.border}`, borderRadius: 20, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '14px 20px', borderBottom: `1px solid ${c.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: 13, color: c.textMuted }}>
            {sorted.length > 0
              ? `Showing ${(page - 1) * ITEMS_PER_PAGE + 1}–${Math.min(page * ITEMS_PER_PAGE, sorted.length)} of ${sorted.length} SKUs`
              : 'No data'}
          </span>
        ...
        {/* Column headers */}
        <div style={{ display: 'grid', gridTemplateColumns: cols, padding: '12px 20px', borderBottom: `1px solid ${c.border}`, background: 'rgba(255,255,255,0.02)', gap: 8, alignItems: 'center' }}>
          <span style={{ fontSize: 10, fontWeight: 700, color: c.textMuted }}>ITEM / SKU</span>
          <span onClick={() => { setSortBy('units'); setPage(1); }} style={{ fontSize: 10, fontWeight: 700, color: sortBy === 'units' ? c.green : c.textMuted, cursor: 'pointer', textAlign: 'right' }}>UNITS {sortBy === 'units' ? '▼' : ''}</span>
        ...
        {paginated.length > 0 ? paginated.map((d, i) => {
          ...
          return (
            <div
              key={d.sku + d.name + i}
              style={{ display: 'grid', gridTemplateColumns: cols, padding: '12px 20px', borderBottom: `1px solid ${c.border}`, gap: 8, alignItems: 'center', background: 'transparent', transition: 'background 0.15s' }}
            ...
            >
              <div style={{ minWidth: 0 }}>
                ...
                <p style={{ margin: '0 0 5px', fontSize: 11, color: c.green }}>{d.sku || '—'}</p>
                <div style={{ height: 3, borderRadius: 2, background: 'rgba(52,211,153,0.1)' }}>
                  <div style={{ height: '100%', width: `${barWidth}%`, background: '#34D399', borderRadius: 2 }} />
                </div>
              </div>
              <span style={{ fontSize: 13, textAlign: 'right', color: c.textMuted }}>{d.units}</span>
          ...
          <div style={{ padding: 60, textAlign: 'center' }}>
            ...
            <p style={{ color: c.textMuted }}>No confirmed sales yet.</p>
          </div>
        )}
