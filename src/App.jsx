...
export default function App() {
  ...
  const handleCsvUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      setCsvImport(prev => ({ ...prev, show: true, data: parsed, preview: true }));
    };
      ...
      <main style={{ flex: 1, padding: '28px 36px', overflowY: 'auto' }}>
        ...
        {page === 'settings' && <div style={{ maxWidth: 550 }}>
          <div style={{ ...cardStyle, padding: 24, marginBottom: 16 }}>
            <h3 style={{ margin: '0 0 16px', fontSize: 14, fontWeight: 700, color: c.textMuted }}>🏪 DEFAULT STOCKX CHANNEL</h3>
            <select 
              value={settings.defaultChannel || 'StockX Standard'} 
              onChange={e => setSettings({ ...settings, defaultChannel: e.target.value })} 
              style={{ ...inputStyle, width: '100%', padding: 12, fontSize: 14, cursor: 'pointer' }}
            >
            ...
            <p style={{ margin: '10px 0 0', fontSize: 11, color: c.textMuted }}>This will be pre-selected when confirming synced sales</p>
          </div>
